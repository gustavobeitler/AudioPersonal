...
public class PlaybackServiceV04 extends Service {
    public static final String ACTION_PLAY_INDEX = "com.gustavo.reproductorsueno.v04.PLAY_INDEX";
    public static final String ACTION_TOGGLE = "com.gustavo.reproductorsueno.v04.TOGGLE";
    public static final String ACTION_NEXT = "com.gustavo.reproductorsueno.v04.NEXT";
    public static final String ACTION_PREVIOUS = "com.gustavo.reproductorsueno.v04.PREVIOUS";
    public static final String ACTION_SETTINGS = "com.gustavo.reproductorsueno.v04.SETTINGS";
    public static final String ACTION_PLAY_NEXT = "com.gustavo.reproductorsueno.v04.PLAY_NEXT";
    public static final String ACTION_SEEK = "com.gustavo.reproductorsueno.v04.SEEK";
    public static final String BROADCAST_STATE = "com.gustavo.reproductorsueno.v04.STATE";
    ...
    private static final String CHANNEL_ID = "radioenlace_player_v04";
    ...
    private final Runnable progressTicker = new Runnable() {
        @Override public void run() {
            ...
            broadcastState(playing, null, null);
            if (playing) handler.postDelayed(this, 750L);
        }
    };
    ...
    @Override public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_PLAY_INDEX.equals(action)) {
            if (checkExpectedOutput()) startTrack(intent.getIntExtra(EXTRA_INDEX, 0));
        } else if (ACTION_TOGGLE.equals(action)) {
            if (checkExpectedOutput()) togglePlayback();
        } else if (ACTION_NEXT.equals(action)) {
            if (checkExpectedOutput()) nextTrack();
        } else if (ACTION_PREVIOUS.equals(action)) {
            if (checkExpectedOutput()) previousTrack();
    ...
    private void togglePlayback() {
        ...
        if (safeIsPlaying()) pausePlayback(null);
        else {
            requestAudioFocus();
            try {
                player.start();
    ...
    private void startTrack(int index) {
        ...
        try {
            ...
            player.setOnPreparedListener(mp -> {
                ...
                mp.setVolume(0f, 0f);
                mp.start();
                fadeToTarget(nightMode ? 2200 : 1100);
                startForeground(NOTIFICATION_ID, buildNotification(true));
                startProgressTicker();
                broadcastState(true, null, null);
            });
    ...
    private void handleTrackError(String missingUri) {
        stopProgressTicker();
        consecutiveErrors++;
        broadcastState(false, "Archivo no disponible. Se salta a la siguiente canción.", missingUri);
    ...
    private void nextTrack() {
        ...
        if (playNextUri != null && !playNextUri.isEmpty()) {
            ...
            String name = playNextName == null || playNextName.isEmpty()
                    ? "Pista siguiente" : playNextName;
            playNextUri = null;
            playNextName = null;
    ...
    private void seekTo(int position) {
        ...
        try {
            ...
            broadcastState(safeIsPlaying(), null, null);
        } catch (Exception ignored) { }
    }
    ...
    private void applyCurrentSettings(boolean smooth) {
        ...
        attachAudioProcessing();
        if (smooth && safeIsPlaying()) fadeToTarget(700);
    ...
    private int safePosition() {
        try { return player == null ? 0 : Math.max(0, player.getCurrentPosition()); }
        catch (Exception ignored) { return 0; }
    }

    private int safeDuration() {
        try { return player == null ? 0 : Math.max(0, player.getDuration()); }
        catch (Exception ignored) { return 0; }
    }

    private Notification buildNotification(boolean playing) {
        Intent openIntent = new Intent(this, MainActivityV04.class);
        Intent previousIntent = new Intent(this, PlaybackServiceV04.class).setAction(ACTION_PREVIOUS);
        Intent toggleIntent = new Intent(this, PlaybackServiceV04.class).setAction(ACTION_TOGGLE);
        Intent nextIntent = new Intent(this, PlaybackServiceV04.class).setAction(ACTION_NEXT);
